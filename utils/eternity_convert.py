...
def eternity_convert(file, config, output, provider_file_enabled=True):
    ...
    all_provider = subs_function.convert_sub(
        "https://raw.githubusercontent.com/Airuop/cross/master/sub/sub_merge_base64.txt", 'clash', "http://0.0.0.0:25500", False, extra_options="&udp=false")
        ...
        for rule in proxy_groups:
            if rule['name'] == rule_name:
                ...
                if "Core" in rule_name:
                    rule.update({'proxies': cardnames[0:2] + all_name[0:full_size]})
